...
class DrugPlugin: IllegalCommodityPlugin() {
    companion object {
        const val STAB_MALUS = -1f
    }

    override fun apply() {
        getPlayerFac().getFactionMarkets().filter { !it.isFreePort }.forEach { it.stability.modifyFlat("ZHCDrugPlugin", STAB_MALUS, "Legal Drugs") }
    }

    override fun unapply() {
        Global.getSector().economy.marketsCopy.forEach { it.stability.unmodify("ZHCDrugPlugin") }
    }
